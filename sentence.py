...
from search import get_data  # Obtiene datos desde SQL Server
...
def generate_embeddings(texts):
    ...
    with torch.no_grad():
        embeddings = embedding_model(**tokens).last_hidden_state.mean(dim=1).cpu().numpy()
...
def ai_search(query, index, names, top_k=5, threshold=0.6):
    ...
    results = [(names[idx], similarities[0][i]) for i, idx in enumerate(indices[0]) if similarities[0][i] > threshold]
...
if __name__ == "__main__":
    ...
    search_queries = ["J. Prze", "I. Peres", "J. Perez", "J Prez", "Peres", "J Qeres", "Z. Erep", "Peerez", "J Peéz"]
