...
class EmbeddingManager:
    ...
    def __init__(
        ...
        """
        Inicializa el EmbeddingManager con un modelo de embeddings, tipo de índice FAISS y opción de medir tiempos.

        Args:
            embeddings_dir (str): Directorio para almacenar embeddings e índices.
            model_type (EmbeddingModelType): Modelo SentenceTransformer a usar.
            index_type (FaissIndexType): Tipo de índice FAISS a utilizar.
            print_times (bool): Si True, imprime tiempos de ejecución.
        """
    ...
    def search(self, query: str, threshold: float = 0.5, k: int = 10) -> pd.DataFrame:
        """Busca los k elementos más similares a la consulta con un umbral de similitud."""
        start_time = time.time()
        ...
        if self.print_times:
            print(
                f"⏱ [EmbeddingManager] Búsqueda total ({query}): {time.time() - start_time:.2f}s"
            )
